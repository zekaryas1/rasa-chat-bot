...
function setUserResponse(message) {
    const user_response = `<img class="userAvatar" src='./static/img/icon-user.svg'><p class="userMsg">${message} </p><div class="clearfix"></div>`;
...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="./static/img/icon-avatar.svg"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const fallbackMsg = "I'm sorry, I don't quite understand. Could you please rephrase?";

            const BotResponse = `<img class="botAvatar" src="./static/img/icon-avatar.svg"/><span class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        if (html.includes("<ul") || html.includes("<ol") || html.includes("<li") || html.includes('<h3')) {
                            ...
                            if (!botResponse) {
                                botResponse = `<img class="botAvatar" src="./static/img/icon-avatar.svg"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
                            }
                        }
 ...
 */
// eslint-disable-next-line no-unused-vars
function customActionTrigger(message) {
    $.ajax({
        url: "http://localhost:5005/webhooks/rest/webhook",
        type: "POST",
        contentType: "application/json",
